// sciroppo
// O(fast but bad) solution using spfa
// builds an explicit graph and then uses spfa
...
int main() {
	...
	while(!q.empty()) {
		...
		for(edge &e : g[curr.first][curr.second]) {
			...
			if(dist[curr.first][curr.second] + e.weight < dist[e.to.first][e.to.second]) {
				dist[e.to.first][e.to.second] = dist[curr.first][curr.second] + e.weight;
				// cout << curr.first << ", " << curr.second << " - " << e.to.first << ", " << e.to.second << " @ " << dist[curr.first][curr.second] << " + " << e.weight << '\n';
				q.push(e.to);
			}
		}
	}
